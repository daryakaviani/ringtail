...
func main() {
	...
	party := sign.NewParty(partyID, r, r_nu, r_xi, uniformSampler)
	...
	if partyID != sign.CombinerID {
		...
		comm.SendVector(writer, sign.CombinerID, masks[partyID])
		...
		for i := 0; i < sign.K; i++ {
			if i != sign.CombinerID {
				reader := bufio.NewReader(*comm.GetSock(i))
				masks[i] = comm.RecvVector(reader, i, sign.N)
		...
		// SIGNATURE FINALIZE
		start := time.Now()
		_, sig, Delta := party.SignFinalize(z, masks, A, b)
